...
function loadSkillsFromDir(dir) {
  ...
  try {
    ...
    console.log(`[AI Tutor] Loaded ${files.length} skill files from ${dir}/: ${files.map(f => f.name).join(', ')}`)
    return files
  } catch (err) {
    console.warn(`[AI Tutor] WARN: Could not read skill directory ${dir}: ${err.message}`)
    return []
  }
}
  ...
  category: z
    ...
    .describe(
      'Which reviewer this section should be assigned to. ' +
        ...
        'conclusion = conclusion / limitations / ethical considerations / acknowledgments / appendix / proofs'
    ),
})
...
function buildSectionMapping(sectionAssignments, sections) {
  ...
  for (const sec of sections) {
    ...
    if (lower.includes('introduction')) fallbackCat = 'introduction'
      ...
      lower.includes('appendix') || lower.includes('acknowledgment') || lower.includes('proof')
    )
...
async function classifyPaper(openai, model, sections) {
  ...
  const paperTypeSkills = loadSkillsFromDir('02_paper_types')
  const skillContext = paperTypeSkills
    .map(s => `--- ${s.name} ---\n${s.content}`)
    .join('\n\n')

  console.log(`[AI Tutor] Phase 1: Skill context for classifier: ${skillContext.length} chars`)
...
Here are the paper type definitions:
${skillContext}`
...
Your task: Read the provided LaTeX text and produce constructive inline comments.
Each comment must reference an EXACT substring from the text (20-200 chars) as highlightText.
The highlightText MUST appear verbatim in the provided text — do not paraphrase or shorten it.
Produce 3-8 high-quality comments. Prefer fewer, deeper comments over many shallow ones.
...
${guidanceInjection}`
  ...
  const logUserPrompt = `Review the following LaTeX text and provide your comments:\n\n${previewText(reviewText)}`
...
export async function runFullReview({
  ...
  console.log('-'.repeat(60))
  console.log(
    `[AI Tutor] Phase 2: Launching ${SUBAGENT_DEFS.length} reviewer subagents in parallel...`
  )
  const phase2Start = Date.now()

  const AGENT_TIMEOUT = 120_000 // 2 minutes per agent
  const subagentPromises = SUBAGENT_DEFS.map(def => {
  ...
  // Prefix all comments with [AI Tutor]
  for (const c of mappedComments) {
    if (!c.comment.startsWith('[AI Tutor]')) {
      c.comment = `[AI Tutor] ${c.comment}`
    }
  }
